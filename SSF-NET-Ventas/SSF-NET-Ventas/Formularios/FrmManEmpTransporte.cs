...
{
    public partial class FrmManEmpTransporte : Form
    {
        // VARIABLES PUBLICAS
        ...
        // OBJETOS LOCALES
        CN_vta_emptra objRegistros = new CN_vta_emptra();
        ...
        // ENTIDADES LOCALES
        BE_VTA_EMPTRA BE_ListaReg = new BE_VTA_EMPTRA();
        BE_VTA_EMPTRA BE_Registro = new BE_VTA_EMPTRA();
        ...
        public FrmManEmpTransporte()
        {
            InitializeComponent();
        }

        private void FrmEmpTransporte_Load(object sender, EventArgs e)
        {
            booAgregando = true;
            CargarCombos();
        ...
        void AsignarEntidad()
        {
            ...
            BE_ListaReg.n_idpro = Convert.ToInt32(CboCliente.SelectedValue);
        ...
        bool CamposOK()
        {
            ...
            //    return booEstado;
            //}
            //if (Convert.ToInt32(CboDep.SelectedValue) == 0)
            //{
        ...
        private void FrmEmpTransporte_Activated(object sender, EventArgs e)
        {
        ...
        private void FrmEmpTransporte_Resize(object sender, EventArgs e)
        {
        ...
        private void CboCliente_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            {
                string strCadenaFiltro = "n_id = " + CboCliente.SelectedValue.ToString()  + "";
                //CboDis.Enabled = true;

                CboDis.SelectedValue = 0;

                DtFiltro = funDatos.DataTableFiltrar(dtCliPro, strCadenaFiltro);

                TxtCodCEN.Text = DtFiltro.Rows[0]["c_numdoc"].ToString();
                TxtDir.Text = DtFiltro.Rows[0]["c_dir"].ToString();

                if (DtFiltro.Rows[0]["n_iddep"].ToString() != "")
                {
                    CboDep.SelectedValue = Convert.ToInt32(DtFiltro.Rows[0]["n_iddep"].ToString());
                }
                else
                {
                    CboDis.SelectedValue = 0;
                ...
                if (DtFiltro.Rows[0]["n_idpro"].ToString() != "")
                {
                    CboPro.SelectedValue = Convert.ToInt32(DtFiltro.Rows[0]["n_idpro"].ToString());
                }
                else
                {
                    CboPro.SelectedValue = 0;
                ...
                if (DtFiltro.Rows[0]["n_iddis"].ToString() != "")
                {
                    CboDis.SelectedValue = Convert.ToInt32(DtFiltro.Rows[0]["n_iddis"].ToString());
                }
                else
                {
                    CboDis.SelectedValue = 0;
