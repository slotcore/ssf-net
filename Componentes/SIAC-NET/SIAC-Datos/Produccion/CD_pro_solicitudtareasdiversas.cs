...
{
    public class CD_pro_solicitudtareasdiversas
    {
        ...
        public bool Listar(int n_IdEmpresa, int n_AnoTrabajo, int n_MesTrabajo)
        {
            ...
            dtLista = xMiFuncion.StoreDTLLenar("pro_solicitudtareasdiversas_select", arrParametros, mysConec);
        ...
        public bool TraerRegistro(Int64 n_IdRegistro)
        {
            ...
            dtRegistro = xMiFuncion.StoreDTLLenar("pro_solicitudtareasdiversas_obtenerregistro", arrParametros, mysConec);
            if (xMiFuncion.IntErrorNumber == 0)
            {
                string[,] arrParametros2 = new string[1, 3] {
                                            {"n_idsol", "System.INT64", n_IdRegistro.ToString()}
                                      };
                dtRegistroDet = xMiFuncion.StoreDTLLenar("pro_solicitudtareasdiversascab_select", arrParametros2, mysConec);
                if (xMiFuncion.IntErrorNumber == 0)
                {
                    dtRegistroDetPer = xMiFuncion.StoreDTLLenar("pro_solicitudtareasdiversasdet_select", arrParametros2, mysConec);
                    if (xMiFuncion.IntErrorNumber != 0)
                    {
                        b_OcurrioError = xMiFuncion.booOcurrioError;
                        c_ErrorMensaje = xMiFuncion.StrErrorMensaje;
        ...
        public bool Eliminar(int n_IdRegistro)
        {
            ...
            { 
                if (xMiFuncion.StoreEjecutar("pro_solicitudtareasdiversascab_delete", arrParametros, mysConec) == true)
                {
                    arrParametros = new string[1, 3] {
                                                {"n_id", "System.INT64", n_IdRegistro.ToString()}
                                          };
                    if (xMiFuncion.StoreEjecutar("pro_solicitudtareasdiversas_delete", arrParametros, mysConec) == true)
                    {
                        booResult = true;
                    }
                    else
        ...
        public bool BuscarProduccionEnSolicitud(int n_idProduccion, int n_IdEmpresa)
        {
            ...
            dtLista = xMiFuncion.StoreDTLLenar("pro_solicitudtareasdiversas_consulta1", arrParametros, mysConec);
        ...
        public bool MarcarRevisado(int n_IdSolicitudTarea, int n_Estado)
        {
            ...
            xMiFuncion.StoreEjecutar("pro_solicitudtareasdiversas_actualizarestadotareaplanilla", arrParametros, mysConec);
