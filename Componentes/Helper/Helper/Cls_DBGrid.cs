...
namespace Helper
{
    ...
    {
        public void DataTable_To_Excel(List<DataTable> lDatos, string pFilePath)
        {
            try
            {
                if (lDatos != null && lDatos.Count > 0)
                {
                    ...
                    using (FileStream stream = new FileStream(pFilePath, FileMode.Create, FileAccess.ReadWrite))
                    {
                        ...
                        foreach (DataTable tabla in lDatos)
                        {
                            if (tabla != null && tabla.Rows.Count > 0)
                            {
                                worksheet = workbook.CreateSheet(tabla.TableName); //<-Usa el nombre de la tabla como nombre de la Hoja
        ...
        public bool DT_List_ExporExcel(List<DataTable> tables)
        {
            System.Windows.Forms.SaveFileDialog objCuadroDialogo = new System.Windows.Forms.SaveFileDialog();
            bool boolExporto = false;
            objCuadroDialogo.Filter = "MS Excel (*.xlsx) |*.xlsx;*.xlsx|(*.xlsx) |*.xlsx|(*.*) |*.*";

            if (objCuadroDialogo.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                DataTable_To_Excel(tables, objCuadroDialogo.FileName);
                boolExporto = true;
            }
