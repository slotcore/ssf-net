...
namespace SSF_NET_Contabilidad.Formularios
{
    public partial class FrmManPercepcion : Form
    {
        ...
        void AsignarEntidad()
        {
            ...
            {
                e_Percepcion.n_id = Convert.ToInt32(DgLista.Columns["n_id"].CellValue(DgLista.Row).ToString());
            }

            e_Percepcion.c_des = TxtDes.Text;
            e_Percepcion.n_tasa = Convert.ToDouble(TxtTasa.Text);
            e_Percepcion.n_idcuecom = Convert.ToInt32(LblidCueCom.Text);
            e_Percepcion.n_idcueven = Convert.ToInt32(LblidCueVen.Text);
        }
        bool CamposOK()
        {
            bool booEstado = true;
