...
namespace SSF_NET_Almacen.Formularios
{
    public partial class FrmIngresoAlmacenMP : Form
    {
        ...
        void Nuevo()
        {
            ...
            int n_idres = Convert.ToInt32(dtResponsables.Rows[0]["n_id"]);
        ...
        void GenerarNumeroLote(int n_Fila, string c_DesItem)
        {
            ...
            if (dtResul.Rows.Count != 0)
            {
                ...
                if (dtLotes.Rows.Count != 0)
                {
                    ...
                    int n_result = Convert.ToInt32(c_numlote.Substring(n_poscaracter + 1, 2));
            ...
            if (dtresult != null)
            {
                if (dtresult.Rows.Count != 0)
                {
                    ...
                    if (l_OCDet.Count != 0)
                    {
                        ...
                        {
                            // Solo se agrega si el tipo es materia prima
                            dtresult = funDatos.DataTableFiltrar(dtItems, "n_id = " + ocDet.n_idite + "");
                            if (dtresult != null)
                            {
                                if (Convert.ToInt32(dtresult.Rows[0]["n_idtipexi"]) == n_idtipexi)
                                {
