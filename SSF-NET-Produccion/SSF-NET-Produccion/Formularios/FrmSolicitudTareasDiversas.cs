...
{
    public partial class FrmSolicitudTareasDiversas : Form
    {
        // VARIABLES PUBLICAS
        ...
        CN_pro_solicitudtareasdiversas objRegistro = new CN_pro_solicitudtareasdiversas();
        ...
        BE_PRO_SOLICITUDTAREASDIVERSAS entTar = new BE_PRO_SOLICITUDTAREASDIVERSAS();
        List<BE_PRO_SOLICITUDTAREASDIVERSASCAB> lstTarCab = new List<BE_PRO_SOLICITUDTAREASDIVERSASCAB>();
        List<BE_PRO_SOLICITUDTAREASDIVERSASDET> lstTarDet = new List<BE_PRO_SOLICITUDTAREASDIVERSASDET>();
        ...
        public FrmSolicitudTareasDiversas()
        {
            InitializeComponent();
        }
        private void FrmSolicitudTareasDiversas_Load(object sender, EventArgs e)
        {
            booAgregando = true;
            CargarCombos();
        ...
        void AsignarEntidad()
        {
            ...
            {
                BE_PRO_SOLICITUDTAREASDIVERSASCAB entTarCab = new BE_PRO_SOLICITUDTAREASDIVERSASCAB();
        ...
        private void FrmSolicitudTareasDiversas_Activated(object sender, EventArgs e)
        {
        ...
        private void FrmSolicitudTareasDiversas_Resize(object sender, EventArgs e)
        {
        ...
        //    dtResult = funDatos.DataTableFiltrar(dtLineas, "n_idrec = " + LblIdReceta.Text + " and n_idpro = " + LblIdProd.Text + " AND n_act = 0");
        ...
        private void emitirGuiaToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            CN_pro_solicitudtareasdiversas objTar = new CN_pro_solicitudtareasdiversas();
        ...
        private void CmdAddPer_Click(object sender, EventArgs e)
        {
            ...
            CN_pla_empleados objEmpleados = new CN_pla_empleados(STU_SISTEMA);
            objEmpleados.STU_SISTEMA = STU_SISTEMA;
            objEmpleados.Consulta1(STU_SISTEMA.EMPRESAID, c_cadIN);
            dtResult = objEmpleados.dtLista;
            objEmpleados = null;
            ...
            if (dtResult != null)
            {
                if (dtResult.Rows.Count != 0)
                {
                    ...
                    double n_valor = Convert.ToDouble(FgTar.GetData(FgTar.Row, 7)) / Convert.ToDouble(FgTar.GetData(FgTar.Row, 5));
                    for (n_row = 0; n_row <= dtResult.Rows.Count - 1; n_row++)
                    {
                        ...
                        FgPer.SetData(FgPer.Rows.Count - 1, 5, n_valor.ToString("0.00"));

                        c_dato = dtResult.Rows[n_row]["n_id"].ToString();
                        FgPer.SetData(FgPer.Rows.Count - 1, 6, c_dato);

                        AgregarPersonaTarea(Convert.ToInt32(dtResult.Rows[n_row]["n_id"]), n_idtarea, n_valor, c_fchini, c_fchfin);
                        n_Fila = n_Fila + 1;
                    }
                }
            }
        }
        void ActualizarPersonaTarea(int n_IdPersona, int n_Idtarea, string c_HoraInicio, string c_HoraTermino, double n_Cantidad)
        {
            int n_row = 0;
        ...
        void AgregarPersonaTarea(int n_IdPersona, int n_Idtarea, double n_Cantidad, string c_HorIni, string c_HorFin)
        {
            BE_PRO_SOLICITUDTAREASDIVERSASDET entTarDet = new BE_PRO_SOLICITUDTAREASDIVERSASDET();
        ...
        void CalcularCosto(int n_fila)
        {
            ...
            List<BE_PRO_SOLICITUDTAREASDIVERSASDET> l_listar = new List<BE_PRO_SOLICITUDTAREASDIVERSASDET>();
            int n_row = 0;
        ...
        {
            if (booAgregando == true) { return; }
            int n_idper = Convert.ToInt32(FgPer.GetData(FgPer.Row, 6));
            int n_idtar = Convert.ToInt32(FgTar.GetData(FgTar.Row, 9));
            ...
            double n_can = Convert.ToDouble(FgPer.GetData(FgPer.Row, 5));

            if ((e.Col == 3) || (e.Col == 4) || (e.Col == 5))
            {
                ActualizarPersonaTarea(n_idper, n_idtar, c_horini, c_horfin, n_can);
            }
            double n_total = 0;
            n_total = funFlex.FlexSumarCol(FgPer, 5, 2, FgPer.Rows.Count - 1);
            FgTar.SetData(FgTar.Row, 7, n_total.ToString("0.00"));
        }
