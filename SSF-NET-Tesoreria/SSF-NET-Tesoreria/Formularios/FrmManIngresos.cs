...
{
    public partial class FrmManIngresos : Form
    {
        // VARIABLES PUBLICAS
        ...
        public FrmManIngresos()
        {
            InitializeComponent();
        }

        private void FrmManIngresos_Load(object sender, EventArgs e)
        {
            booAgregando = true;
            CargarCombos();
        ...
        void DataTableCargar()
        {
            ...
            objRegistros.Listar(STU_SISTEMA.EMPRESAID, STU_SISTEMA.ANOTRABAJO, STU_SISTEMA.MESTRABAJO, 1);
            dtLista = objRegistros.dtLista;

            objFormVis.mysConec = mysConec;                                      // CARGAMOS EL ARRAY CON LOS DATOS PARA LA VISTA DE DgLista
            objFormVis.ObtenerCabeceraLista(70, ref arrCabeceraDg1);

            objForm.mysConec = mysConec;                                    // CARGAMOS LOS DATOS DEL FORMULARIO
            dtForm = objForm.TraerRegistro(70);
            ...
            dtOri = funDatos.DataTableFiltrar(dtOri, "n_tipo = 1");

            objDes.mysConec = mysConec;
            objDes.Listar(STU_SISTEMA.EMPRESAID);
            dtDes = objDes.dtDestino;
            dtDes = funDatos.DataTableFiltrar(dtDes, "n_tipo = 1");
        ...
        void MostarDetalle()
        {
            ...
            for (n_row = 0; n_row <= l_TesOri.Count - 1; n_row++)
            {
                ...
                {
                    c_dato = l_TesOri[n_row].n_imp.ToString("0.00");
                ...
                else
                {
                    n_valor = l_TesOri[n_row].n_imp;
                    n_valor = (n_valor * l_TesOri[n_row].n_tc);
                    FgOriIng.SetData(FgOriIng.Rows.Count - 1, 3, n_valor.ToString("0.00"));
            ...
            for (n_row = 0; n_row <= l_TesDes.Count - 1; n_row++)
            {
                ...
                if (e_Tesoreria.n_idmon == 115)
                {
                    c_dato = l_TesDes[n_row].n_imp.ToString("0.00");
                ...
                else
                {
                    n_valor = l_TesDes[n_row].n_imp;
                    n_valor = (n_valor * l_TesDes[n_row].n_tc);
                    FgDesIng.SetData(FgDesIng.Rows.Count - 1, 3, n_valor.ToString("0.000000"));
        ...
        void MostrarEstadoMes(int n_IdEmpresa, int n_IdMes)
        {
            ...
            if (obj_fungen.EstadoPeriodo(n_IdEmpresa, n_IdMes, 4) == true)
            {
        ...
        void ActivarTool()
        {
            ...
            TooExportarExcel.Enabled = !TooExportarExcel.Enabled;
            ToolSalir.Enabled = !ToolSalir.Enabled;
        }
        void Modificar()
        {
            int n_dongen = Convert.ToInt16(DgLista.Columns["n_dongen"].CellValue(DgLista.Row).ToString());       // OBTENEMOS EL ID DEL REGISTRO QUE SE DESEA ELIMINAR

            if (n_dongen == 2)
            {
                MessageBox.Show("El registro que desea modificar no fue generado en el modulo de ingresos, no se puede modificar este registro", "", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                return;
            }
            n_QueHace = 2;
            Tab1.TabPages[0].Enabled = false;
        ...
        bool EliminarRegistro()
        {
            ...
            if (n_dongen == 2)
            {
                MessageBox.Show("El registro que desea eliminar no fue generado en el modulo de ingresos, no se puede eliminar este registro", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
                return booResult;
            }
        ...
        bool Grabar()
        {
            ...
            if (n_QueHace == 1)
            {
                booResultado = objRegistros.Insertar(e_Tesoreria, l_TesOri, l_TesOriDet, l_TesDes, l_TesDesDet, 1);
            }

            if (n_QueHace == 2)
            {
                booResultado = objRegistros.Actualizar(e_Tesoreria, l_TesOri, l_TesOriDet, l_TesDes, l_TesDesDet, 1);
            }
        ...
        void AsignarEntidad()
        {
            ...
            {
                e_Tesoreria.n_id = Convert.ToInt16(DgLista.Columns["n_id"].CellValue(DgLista.Row).ToString());
            }
        ...
        {
            bool booEstado = true;
            if (Convert.ToDateTime(TxtFchEmi.Text).Month != Convert.ToInt16(CboMeses.SelectedValue))
            {
        ...
        private void FrmManIngresos_Activated(object sender, EventArgs e)
        {
        ...
        private void ToolGrabar_Click(object sender, EventArgs e)
        {
            if (Grabar() == true)
            {
                ...
                if (n_QueHace == 1)
                {
                    Rpta = MessageBox.Show("! El registro se agrego con exito ¡ ¿Desea agregar otro registro? ", "", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                }
                else
                {
        ...
        private void FrmManIngresos_Resize(object sender, EventArgs e)
        {
            Tab_Dimensionar(Tab1, this.Height - 83, this.Width - 18);
        }
        ...
        private void FgOriIng_CellButtonClick(object sender, C1.Win.C1FlexGrid.RowColEventArgs e)
        {
            ...
            if (FgOriIng.Col == 1)
            {
                ...
                dtResul = funDatos.DataTableFiltrar(dtOri, "n_idmon = " + Convert.ToInt16(CboMon.SelectedValue) + "");
        ...
        private void FgOriIng_CellChanged(object sender, C1.Win.C1FlexGrid.RowColEventArgs e)
        {
            ...
            if (FgOriIng.Col == 4)
            {
                double n_imp = Convert.ToDouble(FgOriIng.GetData(FgOriIng.Row, 4).ToString());
                double n_tc = Convert.ToDouble(LblTc.Text);
                n_imp = (n_imp * n_tc);
                FgOriIng.SetData(FgOriIng.Row, 3, n_imp.ToString("0.00"));
            }
            SumarTotOrigen();
            booAgregando = false;
        }
        ...
        private void FgDesIng_CellChanged(object sender, C1.Win.C1FlexGrid.RowColEventArgs e)
        {
            ...
            if (FgDesIng.Col == 4)
            {
                double n_imp = Convert.ToDouble(funFunciones.NulosN(FgDesIng.GetData(FgDesIng.Row, 4)).ToString());
                double n_tc = Convert.ToDouble(LblTc.Text);
                n_imp = (n_imp * n_tc);
                FgDesIng.SetData(FgDesIng.Row, 3, n_imp.ToString("0.00"));
            }
            SumarTotDestino();
            booAgregando = false;
        }
        ...
        private void CmdDelDoc_Click(object sender, EventArgs e)
        {
            ...
            {
                if (l_TesDesDet[n_row].n_iddoc == n_iddoc)
                {
        ...
        void MostraPanel5()
        {
            ...
            LblConcepto.Text = FgDesIng.GetData(FgDesIng.Row, 1).ToString();
            ...
            if (l_TesDesDet.Count != 0)                              // SI SE HA REGISTRADO DOCUMENTOS DE ORIGEN, LOS MOSTRAMOS EN EL DETALLE
            {
                ...
                for (n_fil = 0; n_fil <= l_TesDesDet.Count - 1; n_fil++)
                {
                    ...
                    CN_con_regpercepcion objper = new CN_con_regpercepcion();
                    BE_VTA_VENTAS e_venta = new BE_VTA_VENTAS();
                    BE_CON_REGPERCEPCION e_perc = new BE_CON_REGPERCEPCION();

                    objven.mysConec = mysConec;
                    if (l_TesDesDet[n_fil].n_idlib == 14) 
                    {
                        e_venta = objven.TraerRegistro(l_TesDesDet[n_fil].n_iddoc);
                        c_dato = funDatos.DataTableBuscar(dtCliPro, "n_id", "c_nombre", e_venta.n_idcli.ToString(), "N").ToString();
                    }

                    if (l_TesDesDet[n_fil].n_idlib == 16) 
                    {
                        objper.mysConec = mysConec;
                        objper.TraerRegistro(l_TesDesDet[n_fil].n_iddoc, 1);
                        e_perc = objper.e_Percepcion;
                        c_dato = funDatos.DataTableBuscar(dtCliPro, "n_id", "c_nombre", e_perc.n_idcli.ToString(), "N").ToString();
                    }
                    ...
                    c_dato = funDatos.DataTableBuscar(dtMon, "n_id", "c_codsun", l_TesDesDet[n_fil].n_idmon.ToString(), "N").ToString();
        ...
        void MostraPanel6()
        {
            ...
            if (l_TesOriDet.Count != 0)                              // SI SE HA REGISTRADO DOCUMENTOS DE ORIGEN, LOS MOSTRAMOS EN EL DETALLE
            {
                ...
                {
                    if (Convert.ToInt16(LblIdOri.Text) == l_TesOriDet[n_fil].n_idori)
                    { 
                        FgDocOri.Rows.Count = FgDocOri.Rows.Count + 1;
                        ...
                        c_dato = funDatos.DataTableBuscar(dtMon, "n_id", "c_codsun", Convert.ToInt16(CboMon.SelectedValue).ToString(), "N").ToString();
            ...
            else
            {
                ...
                FgDocOri.SetData(FgOriIng.Row, 3, c_dato);
            }
        ...
        private void CmdAddDoc_Click(object sender, EventArgs e)
        {
            ...
            {
                dtResul = objVentas.DocumentosConSaldo(STU_SISTEMA.EMPRESAID, 0);
            }
            else
            { 
                dtResul = objVentas.DocumentosConSaldo(STU_SISTEMA.EMPRESAID, Convert.ToInt16(LblidCliente.Text));
            }
            ...
            if (dtResul != null)
            {
                if (dtResul.Rows.Count != 0)
                {
                    for (n_row = 0; n_row <= dtResul.Rows.Count - 1; n_row++)
                    {
                        ...
                        double n_tc = Convert.ToDouble(LblTc.Text);
                        int n_idmon = Convert.ToInt16(dtResul.Rows[n_row]["n_idmon"]);

                        if (Convert.ToInt16(CboMon.SelectedValue) == 115)                       // SI ES SOLES
                        {
                            if (n_idmon == 115)
                            {
                                c_dato = Convert.ToDouble(dtResul.Rows[n_row]["n_saldo"]).ToString("0.00");
        ...
        private void CmdAceOriDoc_Click(object sender, EventArgs e)
        {
            ...
            {
                if (funFunciones.NulosC(FgDocOri.GetData(n_fil, 1)) != "")
                {
                    if (funFunciones.NulosC(FgDocOri.GetData(n_fil, 2)) == "")
                    {
                    ...
                    if (Convert.ToDouble(funFunciones.NulosN(FgDocOri.GetData(n_fil, 5))) == 0)
                    {
            ...
            for (n_fil = 2; n_fil <= FgDocOri.Rows.Count - 1; n_fil++)
            {
                n_corgrid = Convert.ToInt16(funFunciones.NulosN(FgDocOri.GetData(n_fil, 8)).ToString());
                if (BuscarEliminarOrigenDoc(Convert.ToInt16(LblIdOri.Text), n_corgrid) == true)
                {
                    n_cor = n_corgrid;
                }
                else
                ...
                e_OriDet.n_idtes = 0;
                e_OriDet.n_idori = Convert.ToInt16(LblIdOri.Text);
                e_OriDet.n_idtipper = 0;
                e_OriDet.n_idmod = 0;
                e_OriDet.n_iddoc = 0;
                e_OriDet.n_idper = 0;
                e_OriDet.n_idtipdoc = Convert.ToInt16(FgDocOri.GetData(n_fil, 7).ToString());
                e_OriDet.c_numser = "";
                ...
                e_OriDet.n_idmon = Convert.ToInt16(CboMon.SelectedValue);
                e_OriDet.n_idmedpag = Convert.ToInt16(FgDocOri.GetData(n_fil, 6).ToString());
        ...
        bool BuscarEliminarOrigenDoc(int n_IdOrigen, int n_cor)
        {
            ...
            for (n_fil = 0; n_fil <= FgDocOri.Rows.Count - 1; n_fil++)
            {
                if (l_TesOriDet[n_fil].n_idori == n_IdOrigen)
                {
                    if (l_TesOriDet[n_fil].n_cor == n_cor)
                    {
                        l_TesOriDet.RemoveAt(n_fil);
                        b_seencontro = true;
                        return b_seencontro;
                    }
                }
        ...
        private void CmdAce_Click(object sender, EventArgs e)
        {
            ...
            int n_cor = 0;
            int n_corgrid;
            int n_idmod = Convert.ToInt16(funDatos.DataTableBuscar(dtDes, "n_id", "n_idmod", LblIdDes.Text, "N"));

            for (n_fil = 2; n_fil <= fgDocCli.Rows.Count - 1; n_fil++)
            {
                n_corgrid = Convert.ToInt16(funFunciones.NulosN(fgDocCli.GetData(n_fil, 13)).ToString());
                if (BuscarEliminarDestinoDoc(Convert.ToInt16(LblIdDes.Text), n_corgrid) == true)
                {
                    n_cor = n_corgrid;
                }
                else
                ...
                e_DesDet.n_idtes = 0;
                e_DesDet.n_iddes = Convert.ToInt16(LblIdDes.Text);
                e_DesDet.n_idtipper = 0;
                e_DesDet.n_idmod = n_idmod;
                e_DesDet.n_iddoc = Convert.ToInt32(fgDocCli.GetData(n_fil, 11)); ;
                e_DesDet.n_idper = 0;
                e_DesDet.n_idtipdoc = Convert.ToInt16(fgDocCli.GetData(n_fil, 14).ToString());
                ...
                e_DesDet.n_idmon = Convert.ToInt16(CboMon.SelectedValue);
                e_DesDet.n_idlib = Convert.ToInt16(fgDocCli.GetData(n_fil, 15).ToString());
                //e_DesDet.n_idmedpag = Convert.ToInt16(fgDocCli.GetData(n_fil, 6).ToString());
            ...
            if (Convert.ToInt16(CboMon.SelectedValue) == 115)
            {
                FgDesIng.SetData(FgDesIng.Row, 3, n_valor.ToString("0.00"));

                n_valor = (n_valor / Convert.ToDouble(LblTc.Text));
                FgDesIng.SetData(FgDesIng.Row, 4, n_valor.ToString("0.00"));
            }
            else
            {
                FgDesIng.SetData(FgDesIng.Row, 4, n_valor.ToString("0.00"));
        ...
        private void CmdDelOri_Click(object sender, EventArgs e)
        {
            ...
            int n_row = 0;
            booAgregando = true;
            int n_idori = Convert.ToInt16(FgOriIng.GetData(FgOriIng.Row, 5));
        ...
        private void CmdDelDes_Click(object sender, EventArgs e)
        {
            ...
            booAgregando = true;
            int n_row = 0;
            int n_iddes = Convert.ToInt16(FgDesIng.GetData(FgDesIng.Row, 5));
            // ELIMINAMOS EL ORIGEN
        ...
        private void CboMeses_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            objRegistros.Listar(STU_SISTEMA.EMPRESAID, STU_SISTEMA.ANOTRABAJO, Convert.ToInt32(CboMeses.SelectedValue), 1);    // CARGAMOS LOS DATOS DEL FORMULARIO
            dtLista = objRegistros.dtLista;
            LblNumReg.Text = (dtLista.Rows.Count).ToString();
        ...
        private void CmdVerAsiento_Click(object sender, EventArgs e)
        {
            ...
            objConta.VerAsiento(STU_SISTEMA.EMPRESAID, STU_SISTEMA.ANOTRABAJO, Convert.ToInt16(CboMeses.SelectedValue), 1, LblNumAsi.Text);
            objConta = null;
        }
        ...
        private void TooExportarExcel_Click(object sender, EventArgs e)
        {
            string c_NomArchivo = STU_SISTEMA.EMPRESARUC + "-TES-ING-" + STU_SISTEMA.ANOTRABAJO.ToString() + Convert.ToInt16(CboMeses.SelectedValue).ToString("00") + ".xls";
        ...
        private void CmdGenAsi_Click(object sender, EventArgs e)
        {
            objRegistros.mysConec = mysConec;
            objRegistros.STU_SISTEMA = STU_SISTEMA;
            if (objRegistros.RegeneraAsientos(STU_SISTEMA.EMPRESAID, STU_SISTEMA.MESTRABAJO, STU_SISTEMA.ANOTRABAJO, 1, 1) == true)
            {
