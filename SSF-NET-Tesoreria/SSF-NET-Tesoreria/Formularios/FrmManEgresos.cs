...
namespace SSF_NET_Tesoreria.Formularios
{
    public partial class FrmManEgresos : Form
    {
        ...
        void ListarItems()
        {
            ...
            objRegistros.Listar(STU_SISTEMA.EMPRESAID, STU_SISTEMA.ANOTRABAJO, Convert.ToInt16(CboMeses.SelectedValue), 2);
        ...
        private void CmdAddDoc_Click(object sender, EventArgs e)
        {
            ...
            if (LblidCliente.Text != "")
            { 
                dtResul = objCompras.DocumentosConSaldo(STU_SISTEMA.EMPRESAID, Convert.ToInt16(LblidCliente.Text));
            }
            booAgregando = true;
            if (dtResul != null)
            {
        ...
        private void CmdAce_Click(object sender, EventArgs e)
        {
            ...
            int n_cor = 0;
            int n_corgrid;
            int n_idmod = Convert.ToInt16(funFunciones.NulosN(funDatos.DataTableBuscar(dtDes, "n_id", "n_idmod", LblIdDes.Text, "N")));
            ...
            for (n_fil = 2; n_fil <= fgDocCli.Rows.Count - 1; n_fil++)
            {
                ...
                e_DesDet.n_idmon = Convert.ToInt16(funDatos.DataTableBuscar(dtMon,"c_simbolo","n_id",c_dato,"C").ToString());
        ...
        void MostraPanel5()
        {
            ...
            if (l_TesDesDet.Count != 0)                              // SI SE HA REGISTRADO DOCUMENTOS DE ORIGEN, LOS MOSTRAMOS EN EL DETALLE
            {
                ...
                for (n_fil = 0; n_fil <= l_TesDesDet.Count - 1; n_fil++)
                {
                    ...
                    //if (Convert.ToInt16(l_TesDesDet[n_fil].n_idtipdoc) == 2)
                    //{ 
                    ...
                    //}
                    //if (Convert.ToInt16(l_TesDesDet[n_fil].n_idtipdoc) == 86)
                    //{
                    ...
                    //}
                    if (Convert.ToInt16(l_TesDesDet[n_fil].n_idlib) == 8)
                    {
                        objcom.mysConec = mysConec;
                        objcom.TraerRegistro(l_TesDesDet[n_fil].n_iddoc);
                        e_compra = objcom.e_Compras;
        ...
        {
            string c_NomArchivo = STU_SISTEMA.EMPRESARUC + "-TES-EGR-" + STU_SISTEMA.ANOTRABAJO.ToString() + Convert.ToInt16(CboMeses.SelectedValue).ToString("00") + ".xls";
