...
{
    public partial class FrmManCompras2 : Form
    {
        // VARIABLES PUBLICAS
        ...
        private void CreateContextMenu()
        {
            ...
            ToolStripMenuItem menuItem = new ToolStripMenuItem("Buscar Doc. Ingreso Registrados");
            menuItem.Click += new EventHandler(menuItem_Click);
            menuItem.Name = "opcion_01";
            menuStrip.Items.Add(menuItem);
            //this.ContextMenuStrip = menuStrip;
            FgDoc.ContextMenuStrip = menuStrip;
        }
        void menuItem_Click(object sender, EventArgs e)
        {
            ToolStripItem menuItem = (ToolStripItem)sender;
            if (menuItem.Name == "opcion_01")
            {
                AgregarMovimientoFacturado();
            }
        }
        void AgregarMovimientoFacturado()
        {
            string c_dato = "";
            ...
            dtResul = objMov.BuscarIngresosFacturados(STU_SISTEMA.EMPRESAID, Convert.ToInt32(LblIdPro.Text));
            if (dtResul != null)
            {
        ...
        private void FrmManCompras2_Load(object sender, EventArgs e)
        {
            b_Agregando = true;
            CargarCombos();
        ...
        void DataTableCargar()
        {
            ...
            dtItems = objItems.Listar(STU_SISTEMA.EMPRESAID, 1, 1);                                        // CARGAMOS TODOS LOS DEPARTAMENTOS
        ...
        void CargarDatosAdicionales()
        {
            ...
            dtTipDoc = funDatos.DataTableFiltrar(dtTipDoc, "n_id NOT IN(8, 9)");
        ...
        void ListarItems()
        {
            ...
            { 
                ...
                dtLista = funDatos.DataTableFiltrar(dtLista, "(n_idtipdoc IN(2,4,5,6,11,13,15,14,16,17,21,38,37,39))");
            }
        ...
        void Mostrardetalle()
        {
            ...
            for (n_row = 0; n_row <= l_Detalle.Count - 1; n_row++)
            {
                ...
                c_dato = l_Detalle[n_row].n_idtipafeigv.ToString();
                if (c_dato == "10")
                {
                    c_dato = l_Detalle[n_row].n_preuni.ToString("0.000000");                    // PRECIO UNITARIO DEL ITEM
                    FgItems.SetData(FgItems.Rows.Count - 1, 4, c_dato);
                    FgItems.SetData(FgItems.Rows.Count - 1, 5, "0.00");
                }
                else
                {
                    c_dato = l_Detalle[n_row].n_preuni.ToString();  
        ...
        void AsignarEntidad()
        {
            ...
            for (n_row = 2; n_row <= FgItems.Rows.Count - 1; n_row++)
            {
                ...
                { 
                    ...
                    if (Convert.ToDouble(FgItems.GetData(n_row,4)) != 0)
                    {
                        e_Det.n_preunibru = Convert.ToDouble(FgItems.GetData(n_row,4));         // IMPORTE BRUTO DEL ITEM
                        e_Det.n_idtipafeigv = 10;
                    }
                    
                    if (Convert.ToDouble(FgItems.GetData(n_row,5)) != 0)
                    {
                        e_Det.n_preunibru = Convert.ToDouble(FgItems.GetData(n_row, 5));         // IMPORTE BRUTO DEL ITEM
        ...
        bool CamposOK()
        {
            ...
            if (Convert.ToDateTime(TxtFchEmi.Text).Year < STU_SISTEMA.ANOTRABAJO)
            {
            }
            else
            { 
                if (Convert.ToDateTime(TxtFchEmi.Text).Month > STU_SISTEMA.MESTRABAJO)
                {
                    MessageBox.Show("¡ La fecha del documento no puede ser mayor al periodo de trabajo !", "", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                    booEstado = false;
                    TxtFchEmi.Focus();
                    return booEstado;
                }
            ...
            for(n_row=2; n_row <= FgItems.Rows.Count-1; n_row++)
            {
                if (funFunciones.NulosC(FgItems.GetData(n_row, 9)) != "")
                { 
                    dtres = funDatos.DataTableFiltrar(dtpcite,"n_iditem = " + FgItems.GetData(n_row, 9).ToString() + "");
        ...
        private void FrmManCompras2_Activated(object sender, EventArgs e)
        {
        ...
        private void OptOtrPro_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void CmdVerDoc_Click(object sender, EventArgs e)
        {
            ...
            dtResul = objMov.BuscarMovimientosProcesados(STU_SISTEMA.EMPRESAID, Convert.ToInt32(LblIdPro.Text), c_dato); 
        ...
        private void ToolImpoApertura_Click(object sender, EventArgs e)
        {
            ...
            {
                objRegistros.mysConec = mysConec;
                if (objRegistros.ImportarApertura(STU_SISTEMA.EMPRESAID, STU_SISTEMA.ANOTRABAJO - 1, 8) == true)
                {
                    ListarItems();
                    DgLista.DataSource = dtLista;
