...
{
    public partial class FrmManLiquidacionCompra : Form
    {
        // VARIABLES PUBLICAS
        ...
        CN_pro_rendimiento o_Ren = new CN_pro_rendimiento();
        ...
        DataTable dtRemMP = new DataTable();
        ...
        public FrmManLiquidacionCompra()
        {
            InitializeComponent();
        }

        private void FrmManLiquidacionCompra_Load(object sender, EventArgs e)
        {
            b_Agregando = true;
            CargarCombos();
        ...
        void DataTableCargar()
        {
            ...
            objFormVis.ObtenerCabeceraLista(84, ref arrCabeceraDg1);

            objForm.mysConec = mysConec;                                    // CARGAMOS LOS DATOS DEL FORMULARIO
            dtForm = objForm.TraerRegistro(84);
            ...
            o_Ren.mysConec = mysConec;
            o_Ren.Listar(0,1);
            dtRemMP = o_Ren.dtLista;
        }
        void ListarItems()
        {
            objRegistros.mysConec = mysConec;                                    // CARGAMOS LOS DATOS DEL FORMULARIO
            if (objRegistros.Listar(STU_SISTEMA.EMPRESAID, STU_SISTEMA.MESTRABAJO, STU_SISTEMA.ANOTRABAJO, 1) == true)
            {
                dtLista = objRegistros.dtLista;
                dtLista = funDatos.DataTableFiltrar(dtLista, "(n_idtipdoc IN(5))");
            }
        ...
        void VerRegistro(int n_IdRegistro)
        {
            ...
            LblIdOrdProd.Text = e_Cabecera.n_idordpro.ToString();

            CN_alm_movimientos o_mov = new CN_alm_movimientos();
            BE_ALM_MOVIMIENTOS e_mov = new BE_ALM_MOVIMIENTOS();
            o_mov.mysConec = mysConec;
            e_mov = o_mov.TraerRegistro(e_Cabecera.n_idordpro);
            TxtNumOrdPro.Text = e_mov.c_numser + "-" + e_mov.c_numdoc;
        ...
        void AsignarEntidad()
        {
            ...
            e_Cabecera.n_tipcom = 1;
            e_Cabecera.n_idordpro = Convert.ToInt32(LblIdOrdProd.Text);
        ...
        bool CamposOK()
        {
            ...
            {
                //if (objRegistros.BuscarDocumento(Convert.ToInt32(LblIdPro.Text), Convert.ToInt32(CboTipDoc.SelectedValue), TxtNumSer.Text, TxtNumDoc.Text, e_Cabecera.n_id) == true)
                //{
                //    MessageBox.Show("¡ La " + CboTipDoc.Text.Trim() + " Nº " + TxtNumSer.Text + "-" + TxtNumDoc.Text + " ya existe, ingrese otro !", "", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                //    booEstado = false;
                //    TxtNumDoc.Focus();
            ...
            if (Convert.ToInt32(LblIdOrdProd.Text) == 0)
            {
                MessageBox.Show("¡ No ha indicado la orden de produccion !", "", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                booEstado = false;
                TxtNumOrdPro.Focus();
                return booEstado;
            }

            string c_dain = funFlex.Flex_CadenaIN(FgItems, 9, 2);
            if (objRegistros.TieneCtaContable(c_dain, STU_SISTEMA.EMPRESAID) == false) { booEstado = false; return booEstado; }
        ...
        private void FrmManLiquidacionCompra_Activated(object sender, EventArgs e)
        {
        ...
        private void FrmManLiquidacionCompra_Resize(object sender, EventArgs e)
        {
            Tab_Dimensionar(Tab1, this.Height - 83, this.Width - 18);
        }
        ...
        private void FgDoc_CellButtonClick(object sender, C1.Win.C1FlexGrid.RowColEventArgs e)
        {
            ...
            if (FgDoc.Col == 1)
            {
                ...
                dtResul = o_pro.BuscarProduccionPendLiquidacion(STU_SISTEMA.EMPRESAID);

                if (dtResul != null)
                {
                    if (dtResul.Rows.Count != 0)
                    {
                        c_dato = dtResul.Rows[0]["c_ordprotipdocabr"].ToString();           // LMOSTRAMOS LA ABREVIATURA DEL TIPO DE DOCUMENTO
                        ...
                        c_dato = dtResul.Rows[0]["n_proidtipdoc"].ToString();            // MOSTRAMOS EL ID DEL TIPO DE DOCUMENTO
                        FgDoc.SetData(FgDoc.Row, 4, c_dato);

                        n_porren =  Convert.ToDouble(dtResul.Rows[0]["n_renpor"]);

                        AdicionarItem(Convert.ToInt32(dtResul.Rows[0]["n_idnoting"]), Convert.ToDouble(dtResul.Rows[0]["n_conmp"]), n_porren);
                    }
                }
                b_Agregando = false;
            }
        ...
        void AdicionarItem(int n_IdMovimiento, double n_CantidadDespuesRendimiento, double n_Rendimiento)
        {
            CN_alm_movimientos objMov = new CN_alm_movimientos();
            ...
            {
                if (entMov.lst_items.Count == 1)
                {
                    entMov.lst_items[0].n_canconmp = n_CantidadDespuesRendimiento;
                }
                //int n_iditem = entMov.lst_items[0].n_idite;
                //dtres = funDatos.DataTableFiltrar(dtRemMP, "n_idmatpri = " + n_iditem.ToString() + "");
                //if (dtres.Rows.Count != 0)
                //{
                //    n_valpor = Convert.ToDouble(dtres.Rows[0]["n_porren"]);
                //}
        ...
        void AdicionarLista(List<BE_ALM_MOVIMIENTOSDET_CONSULTA> lstDetalle)
        {
            ...
            for (n_row = 0; n_row <= lstDetalle.Count - 1; n_row++)
            {
                ...
                for (n_fil = 0; n_fil <= l_Detalle.Count - 1; n_fil++)
                {
                    ...
                    {
                        l_Detalle[n_fil].n_canpro = (l_Detalle[n_fil].n_canpro + lstDetalle[0].n_canconmp);
                        b_SeEncontro = true;
                    }
                }
                if (b_SeEncontro == false)
                {
                    ...
                    entComDet.n_canpro = lstDetalle[n_row].n_canconmp;
        ...
        private void CmdBusOrdPro_Click(object sender, EventArgs e)
        {
            ...
            //CN_pro_produccion o_pro = new CN_pro_produccion();
            //o_pro.mysConec = mysConec;
            //dtresult = o_pro.BuscarProduccionPendLiquidacion(STU_SISTEMA.EMPRESAID);
            //if (dtresult.Rows.Count != 0)
            //{
            //    FgItems.Rows.Count = 2;
            //    FgDoc.Rows.Count = 2;
            ...
            //    LblIdOrdProd.Text = dtresult.Rows[0]["n_id"].ToString();
            //    TxtNumOrdPro.Text = dtresult.Rows[0]["c_numdoc"].ToString();
            //    FgDoc.SetData(2, 1, dtresult.Rows[0]["c_notingtipdoc"].ToString());
            //    FgDoc.SetData(2, 2, dtresult.Rows[0]["c_numdoc"].ToString());
            //    FgDoc.SetData(2, 3, dtresult.Rows[0]["n_idnoting"].ToString());
            //    FgDoc.SetData(2, 4, dtresult.Rows[0]["n_idtipdoc"].ToString());

            //    AdicionarItem(Convert.ToInt32(dtresult.Rows[0]["n_idnoting"]));
            //}
        }
    }
}
